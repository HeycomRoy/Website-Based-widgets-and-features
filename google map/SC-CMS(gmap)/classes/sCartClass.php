...
//		echo 'cart: '.$_SESSION['cart'];
	...
	public function doProcessOrder($odb)
	{
		//    The process payment depends on a third party payment gateway which return a result object e.g. $result
		$result = array();
		$result[] = true;  			//  say first element is status, set to true for now for successful
		$result[] = '123NTST456';   //  say second element is receipt number
		if ($result[0]) {
			$totalSale = $this->getTotalValueFromSession();
			$products = $this->getCartData($odb);
			$odb->createOrder($result, $products, $totalSale);
			unset($_SESSION['cart']);
			$status = 'Thank you. Your transaction with total $ '.$totalSale.
					  ' has been completed with receipt number '.$result[1].'.';
		}
		else {
			$status = 'Sorry we are unable to process this transaction';
		}
		return $status;
	}
}
